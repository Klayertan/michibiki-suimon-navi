...
| **入力** | QZSS/GNSSで実測した圃場境界 ＋ 生育ステージ ＋ 現在の水位 |
| **処理** | 農学的根拠モデル（生育ステージ → 管理方法・目標水深）＋ 圃場ジオメトリ |
| **出力** | 推奨する管理状態・目標水深との差・必要湛水量の目安（出典つき） |
...
flowchart TD
    ...
    Manual["水位の手入力<br/>符号つき valueMm・田面基準"] --> Current["現在の水位（符号つき）"]
    ...
    Weather["気象データ 降水量<br/>実装済み"] --> Gate["水門アドバイス"]
    Gate --> Decision
    ET["蒸発散 ETc = Kc x ETo<br/>将来 - 未実装"] -.-> Engine
```
